...
# df = pd.read_csv("./data/two/telecom_dataset.csv")
...
def generate_summary(conversation_text):
    final_prompt = SUMMARY_PROMPT.format(conversation=conversation_text)
...
for idx, row in tqdm(df.head(1000).iterrows(), total=1000, desc="Generating summaries"):
    if pd.isna(df.at[idx, 'summary']):
        try:
...
df.to_csv("datasummaries.csv", index=False)
print("Summaries generated and saved!")